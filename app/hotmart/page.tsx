  ...
  return (
    <section className="flex flex-col items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 flex-wrap md:gap-6 pt-5  md:py-4">
        {/*gap-9 flex flex-wrap justify-center*/produtosHotmart.map((item) => {
          return (
            <div
              key={item.id}
            ...
            >
              ...
              <div className="p-4 ">
                ...
                <p className={`text-sm font-normal ${item.descricao.length < 20 ? "h-20" : ""}  md:inline text-gray-500 transition-all duration-500 leading-5 mb-5 overflow-y-scroll scrollbar-hide`}>
                  {item.descricao}
                </p>
                <a
                  href={item.linkDeCompra}
                  target="_blank"
                  className="bg-indigo-600 block shadow-sm rounded-md md:rounded-full my-3 py-2 px-5 text-xs text-white font-semibold"
                >
                  Ver produto...
                </a>
              </div>
